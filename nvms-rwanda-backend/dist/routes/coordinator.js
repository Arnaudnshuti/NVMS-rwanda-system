...
async function loadScopedVolunteer(req, userId) {
    ...
    if (!volunteer)
        return { status: 404, error: "Volunteer not found" };
    if (me.role === "coordinator" && (!me.district || volunteer.district !== me.district)) {
        return { status: 403, error: "You may only manage volunteers in your district." };
    }
    return { status: 200, me, volunteer };
}
coordinatorRouter.get("/volunteers", async (req, res) => {
    ...
    const districtWhere = me.role === "coordinator" && me.district ? { district: me.district } : {};
    const users = await prisma.user.findMany({
        where: {
            role: "volunteer",
            ...districtWhere,
            ...(status ? { verificationStatus: status } : {}),
            ...(trustStatus ? { profileTrustStatus: trustStatus } : {}),
            ...(q
                ? {
                    OR: [
...
coordinatorRouter.get("/volunteers/:userId", async (req, res) => {
...
coordinatorRouter.patch("/volunteers/:userId", async (req, res) => {
...
coordinatorRouter.delete("/volunteers/:userId", async (req, res) => {
    const scoped = await loadScopedVolunteer(req, req.params.userId);
    if (scoped.status !== 200)
        return res.status(scoped.status).json({ error: scoped.error });
...
coordinatorRouter.patch("/volunteers/:userId/verification", async (req, res) => {
...
coordinatorRouter.patch("/volunteers/:userId/trust", async (req, res) => {
...
function categoryResourceKit(category, slotsTotal, requiredSkills) {
    ...
    const categoryItems = categoryKey.includes("health")
        ? [
...
coordinatorRouter.get("/resources", async (req, res) => {
    const me = await prisma.user.findUnique({ where: { id: req.userId } });
    if (!me)
        return res.status(401).json({ error: "Unauthorized" });
    const programWhere = me.role === "coordinator"
...
coordinatorRouter.get("/smart-match", async (req, res) => {
...
coordinatorRouter.post("/messages", async (req, res) => {
...
coordinatorRouter.get("/deployments", async (req, res) => {
    const me = await prisma.user.findUnique({ where: { id: req.userId } });
    if (!me)
        return res.status(401).json({ error: "Unauthorized" });
    const where = me.role === "coordinator"
...
coordinatorRouter.get("/activity-logs", async (req, res) => {
    const me = await prisma.user.findUnique({ where: { id: req.userId } });
    if (!me)
        return res.status(401).json({ error: "Unauthorized" });
...
coordinatorRouter.patch("/activity-logs/:id", async (req, res) => {
...
coordinatorRouter.post("/deployments/assign", async (req, res) => {
    ...
    if (exists)
        return res.status(409).json({ error: "Volunteer already assigned to this program." });
