...
async function buildMyCertificates(userId) {
...
meRouter.get("/certificates", async (req, res) => {
    res.json(await buildMyCertificates(req.userId));
});
meRouter.get("/certificates/:assignmentId/pdf", async (req, res) => {
    ...
    if (cert.status !== "issued")
        return res.status(400).json({ error: cert.reason });
    ...
    doc.moveDown(1.2);
    doc.font("Helvetica").fontSize(11).fillColor("#334155").text(`${cert.hoursServed} approved hour(s), served in ${cert.programDistrict} from ${cert.startDate} to ${cert.endDate}.`, { align: "center", width });
    doc.moveDown(2);
