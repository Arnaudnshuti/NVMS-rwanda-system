...
async function buildNationalReport(filterDistrict?: string) {
  ...
  const volunteersMap = new Map<string, number>();
  for (const row of volunteersByDistrict) {
    if (row.district) volunteersMap.set(row.district, row._count._all);
  }
...
adminRouter.get("/analytics", async (_req, res) => {
  ...
  const monthlyMap = new Map<string, { month: string; volunteers: Set<string>; hours: number }>();
  for (const l of await prisma.activityLog.findMany({ where: { status: "approved" }, select: { volunteerId: true, date: true, hours: true } })) {
    ...
    const row = monthlyMap.get(key) ?? { month: monthFmt.format(l.date), volunteers: new Set<string>(), hours: 0 };
...
adminRouter.get("/reports/export", async (req, res) => {
  ...
  if (format === "xlsx") {
    ...
    if (sections.has("districts")) addSheet(wb, "Districts", districtRows);
    if (sections.has("volunteers")) addSheet(wb, "Volunteers and applications", volunteerRows);
    if (sections.has("programs")) addSheet(wb, "Programs", programRows);
  ...
  if (format === "pdf") {
    ...
    const row = (cols: Array<{ text: string; x: number; width: number; bold?: boolean }>, y: number) => {
      cols.forEach((c) => {
  ...
  const csv = [header.join(","), ...csvRows.map((r) => header.map((h) => JSON.stringify((r as Record<string, unknown>)[h] ?? "")).join(","))].join("\n");
