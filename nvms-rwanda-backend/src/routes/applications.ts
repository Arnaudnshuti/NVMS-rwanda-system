...
function serializeApplication(a: {
  ...
  volunteer: { email: string; name: string; district: string | null };
}) {
  return {
    id: a.id,
...
applicationsRouter.patch("/:id", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (me.role === "volunteer") {
    if (app.volunteerId !== me.id) return res.status(403).json({ error: "Forbidden" });
